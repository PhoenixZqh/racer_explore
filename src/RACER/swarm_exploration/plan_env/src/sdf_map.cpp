...
{
    mp_.reset(new MapParam);        //地图参数
    md_.reset(new MapData);         // 地图数据
    mr_.reset(new MapROS);          // ROS接口
    mm_.reset(new MultiMapManager); // 多地图管理

    // Params of map properties
    double x_size, y_size, z_size;
    nh.param("sdf_map/resolution", mp_->resolution_, -1.0);                   // 地图分辨率
    nh.param("sdf_map/map_size_x", x_size, -1.0);                             // 地图 x 轴尺寸
    nh.param("sdf_map/map_size_y", y_size, -1.0);                             // 地图 y 轴尺寸
    nh.param("sdf_map/map_size_z", z_size, -1.0);                             // 地图 z 轴尺寸
    nh.param("sdf_map/obstacles_inflation", mp_->obstacles_inflation_, -1.0); // 障碍物膨胀距离
    nh.param("sdf_map/local_bound_inflate", mp_->local_bound_inflate_, 1.0);  // 本地边界膨胀
    nh.param("sdf_map/local_map_margin", mp_->local_map_margin_, 1);          // 本地地图边距
    nh.param("sdf_map/ground_height", mp_->ground_height_, 1.0);              // 地面高度
    nh.param("sdf_map/default_dist", mp_->default_dist_, 5.0);                // 默认距离
    nh.param("sdf_map/optimistic", mp_->optimistic_, true);                   // 是否使用乐观更新
    nh.param("sdf_map/signed_dist", mp_->signed_dist_, false);                // 是否计算签名距离
    nh.param("sdf_map/no_drone_1", mp_->no_drone_1_, false);                  // 是否忽略无人机 1
    ...
    nh.param("sdf_map/p_hit", mp_->p_hit_, 0.70);                             // 体素被击中的概率
    nh.param("sdf_map/p_miss", mp_->p_miss_, 0.35);                           // 体素未被击中的概率
    nh.param("sdf_map/p_min", mp_->p_min_, 0.12);                             // 最小概率
    nh.param("sdf_map/p_max", mp_->p_max_, 0.97);                             // 最大概率
    nh.param("sdf_map/p_occ", mp_->p_occ_, 0.80);                             // 占用阈值
    nh.param("sdf_map/max_ray_length", mp_->max_ray_length_, -0.1);           // 最大光线长度
    nh.param("sdf_map/virtual_ceil_height", mp_->virtual_ceil_height_, -0.1); // 虚拟天花板高度
    ...
    md_->occupancy_buffer_ = vector<double>(buffer_size, mp_->clamp_min_log_ - mp_->unknown_flag_); // 占用概率
    md_->occupancy_buffer_inflate_ = vector<char>(buffer_size, 0);                                  // 膨胀占用标志
    md_->distance_buffer_neg_ = vector<double>(buffer_size, mp_->default_dist_);                    // 负距离缓冲区
    md_->distance_buffer_ = vector<double>(buffer_size, mp_->default_dist_);                        // 距离缓冲区
    md_->count_hit_and_miss_ = vector<short>(buffer_size, 0);                                       // 击中和未击中计数
    md_->count_hit_ = vector<short>(buffer_size, 0);                                                // 击中计数
    md_->count_miss_ = vector<short>(buffer_size, 0);                                               // 未击中计数
    md_->flag_rayend_ = vector<char>(buffer_size, -1);                                              // 光线终点标志
    md_->flag_visited_ = vector<char>(buffer_size, -1);                                             // 访问标志
    md_->tmp_buffer1_ = vector<double>(buffer_size, 0);                                             // 临时缓冲区 1
    md_->tmp_buffer2_ = vector<double>(buffer_size, 0);                                             // 临时缓冲区 2
    md_->raycast_num_ = 0;                                                                          // 光线投射计数
    md_->reset_updated_box_ = true;                                                                 // 重置更新边界标志
...
void SDFMap::updateESDF3d()
{
    ...
    for (int x = min_esdf[0]; x <= max_esdf[0]; x++)
        ...
        {
            fillESDF([&](int y) { return md_->tmp_buffer1_[toAddress(x, y, z)]; }, [&](int y, double val) { md_->tmp_buffer2_[toAddress(x, y, z)] = val; }, min_esdf[1], max_esdf[1], 1);
        }
    for (int y = min_esdf[1]; y <= max_esdf[1]; y++)
        for (int z = min_esdf[2]; z <= max_esdf[2]; z++)
        {
            fillESDF([&](int x) { return md_->tmp_buffer2_[toAddress(x, y, z)]; }, [&](int x, double val) { md_->distance_buffer_[toAddress(x, y, z)] = mp_->resolution_ * std::sqrt(val); }, min_esdf[0], max_esdf[0], 0);
        }
    ...
    if (mp_->signed_dist_)
    {
        ...
        for (int x = min_esdf[0]; x <= max_esdf[0]; x++)
            for (int y = min_esdf[1]; y <= max_esdf[1]; y++)
            {
                fillESDF(
                    [&](int z) {
                        return md_->occupancy_buffer_inflate_[x * mp_->map_voxel_num_(1) * mp_->map_voxel_num_(2) + y * mp_->map_voxel_num_(2) + z] == 0 ?
                                   0 :
                                   std::numeric_limits<double>::max();
                    },
                    [&](int z, double val) { md_->tmp_buffer1_[toAddress(x, y, z)] = val; }, min_esdf[2],
        ...
        for (int y = min_esdf[1]; y <= max_esdf[1]; y++)
            for (int z = min_esdf[2]; z <= max_esdf[2]; z++)
            {
                fillESDF([&](int x) { return md_->tmp_buffer2_[toAddress(x, y, z)]; }, [&](int x, double val) { md_->distance_buffer_neg_[toAddress(x, y, z)] = mp_->resolution_ * std::sqrt(val); }, min_esdf[0], max_esdf[0], 0);
            }
...
void SDFMap::inputPointCloud(const pcl::PointCloud<pcl::PointXYZ> &points, const int &point_num, const Eigen::Vector3d &camera_pos)
{
