...
public:
    ...
    void allocateGrids(const vector<Eigen::Vector3d> &positions, const vector<Eigen::Vector3d> &velocities, const vector<vector<int>> &first_ids, const vector<vector<int>> &second_ids, const vector<int> &grid_ids, vector<int> &ego_ids, vector<int> &other_ids);

    // Find optimal tour visiting unknown grid
    bool findGlobalTourOfGrid(const vector<Eigen::Vector3d> &positions, const vector<Eigen::Vector3d> &velocities, vector<int> &ids, vector<vector<int>> &others, bool init = false);
    ...
    double computeGridPathCost(const Eigen::Vector3d &pos, const vector<int> &grid_ids, const vector<int> &first, const vector<vector<int>> &firsts, const vector<vector<int>> &seconds, const double &w_f);

    shared_ptr<ExplorationData> ed_;                 //存放本机和伙伴的任务和状态
    shared_ptr<ExplorationParam> ep_;                //管理搜索规律
    shared_ptr<FastPlannerManager> planner_manager_; //规划管理
    shared_ptr<FrontierFinder> frontier_finder_;     //边界相关的处理
    shared_ptr<HGrid> hgrid_;                        // 划分网格相关
    shared_ptr<SDFMap> sdf_map_;                     //获取sdfmap数据
