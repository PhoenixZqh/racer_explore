...
void visCallback(const ros::TimerEvent &e)
{
    ...
    displayTrajWithColor(traj_cmd_, 0.05, PlanningVisualization::getColor((drone_id_ - 1) / double(drone_num_)), pub_traj_id_);
...
void cmdCallback(const ros::TimerEvent &e)
{
    ...
    ROS_WARN_THROTTLE(1.0, "Drone %d, [%lf,%lf,%lf,%lf], [time, length, vel, energy]", drone_id_, flight_t, len, len / flight_t, energy);
    ...
    cmd.yaw = yaw;
    cmd.yaw_dot = yawdot;
    pos_cmd_pub.publish(cmd); //FIXME: 这里是发送位置的指令， 注意映射到对应的控制话题上
